...
Full public + internal endpoint reference: `docsite/api.md` (master, generated by
`scripts/edop/docsite/generate_api_guide.py` from live route signatures + docstrings — edit
those, not the .md directly; served at `/docs/api/`). Swagger UI is at `/api/schema` (moved
there from `/docs` on 2026-08-04 to free that route for MkDocs).
...
Standing rules for every notebook live in `notebooks/CLAUDE.md` — loads automatically when
working under `notebooks/`. Consult it before writing or editing any cell.
