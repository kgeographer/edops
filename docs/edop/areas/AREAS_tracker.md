...
Engine **assembled and whole**; now being extended across the neighborhood and support axes off the buffer-built core (everything downstream of the weighted basin set is reused; only resolvers are neighborhood-specific). **WO12** (L8 buffer — clean MAUP read), **WO14** (single-basin — first *meaningful-boundary* neighborhood, validated end-to-end against v0.3), **WO15** (area-weighted cell weighting), and **WO16** (basin-ring spatial exploration) done. The engine has been exercised at both ends of cardinality: n=1 (single-basin) and 74 basins (L8 buffer). **Neighborhood taxonomy governs the path:** meaningful-boundary neighborhoods (`basin`, ring-expansion, polity) follow something real and can't silently clip an extreme edge unit — they are the dashboard/headline path; arbitrary-boundary neighborhoods (`buffer`, bbox) are analyst-drawer only. **WO16 confirmed:** ST_Touches is clean on both basin06 and basin08 across three fixtures (Timbuktu, Rome, Baghdad); all shared geometries are proper edges (ST_MultiLineString), no slivers, ring sizes 5–13 at L06 and 7 at L08. Weight policy (equal / area-proportional / border-length) surfaced but not decided — in deferred register. **Next:** basin-ring resolver design and engine promotion (WO17).
