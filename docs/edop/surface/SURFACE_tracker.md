...
- **Last updated:** 2026-07-01 (WO1 exemplar payloads captured; inspection findings in exemplars/README.md)
...
Phase opened 2026-06-30, immediately after Areas closed with WO22 (`/area` endpoint stub).
Engine is whole and endpoint-wired: `areal_signature` (point/buffer on `/signature`) and
`GET /api/area` (polity-by-name+year). First Surface work is a capability-gap analysis
before any page is built.

**Next:** sandbox capability-gap analysis (below), then new-page spec informed by it.
