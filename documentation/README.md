...
| `edops_schema.json` | EDOPS signature schema v0.4 (regenerated 2026-08-16 against live output) — documents the `/api/signature` response structure for both default and `&flat` modes, with Timbuktu example values. `/api/area` and `/api/areas` share the same `signature_bands` envelope (documented at `docsite/api.md` instead). |
